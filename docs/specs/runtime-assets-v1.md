...
| `atlases` | Stable atlas ID, relative PNG path, dimensions, sampling, and optional mips |
...
and sampled using their declared `nearest` or `linear` mode. Optional
deterministic `mips[]` entries declare contained images, dimensions, and density
scales. Source rectangles must fit their atlas. Animation frames and tile-set
entries must reference declared sprites.
...
| `sprout.display_label(text, x, y, width, height, r, g, b, a?)` | Submit one bounded ExtraBold display label from the shared font atlas |
| `sprout.rounded_rect(x, y, width, height, radius, r, g, b, a?)` | Submit one filled rounded panel with a bounded corner radius |
...
selects the closest declared source density that does not undersample the
requested draw size, falling back to the nearest available density. Source UVs
are adjusted to the selected mip while sprite geometry and anchors remain
stable.
...
each game's explicit palette. `--check` compares generated bytes with the tracked
...
`tools/generate_arcade_mips.py --check` provides the same stale-output gate for
tracked nearest-neighbour mip derivatives.
