...
For the implemented Windows native-runtime preview, the runtime owns `GameStarted` and `GameExited`; package code may report only `AchievementUnlocked` and `LevelCompleted`. The preview does not yet persist these events. The target engine lifecycle keeps host/policy events runtime-owned and adds bounded game facts only through versioned payload schemas; see [Runtime Game Lifecycle](../runtime/game-lifecycle.md).
