...
The current 640×480 presentation provides offline locale choices, two built-in parent portraits, optional staged image import and cropping, controller-based parent PIN creation, an optional built-in child profile with conservative policy references, and save-and-exit at every screen. It transitions directly to the persisted profile selector after review. Free-form names, Wi-Fi, library discovery, and connectors remain separate unfinished milestone work; the UI labels those steps honestly and does not create placeholder data.
