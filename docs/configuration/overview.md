...
Configuration is validated before activation, written atomically, and snapshotted. Repeated startup failure or invalid configuration must offer last-known-good rollback, connector isolation, launcher-only reset, and stock-Onion recovery without deleting saves. The implemented [startup-health core](../specs/startup-health.md) can request recovery after three unfinished attempts, but no launcher recovery presentation or stock-Onion startup path is wired yet.
