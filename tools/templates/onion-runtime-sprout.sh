...
sprout_containment_enabled() {
    [ -f /mnt/SDCARD/App/Sprout/.containment-enabled ] &&
        ! [ -f /mnt/SDCARD/sprout-dev/maintenance/allow-stock-onion ] &&
        ! [ -f /tmp/sprout-stock-onion-session ]
}

queue_sprout() {
    staged="$sysdir/.sprout-launch-command-$$"
    printf '%s\n' '#!/bin/sh' 'export SPROUT_AUTO_LAUNCH=1' \
        'exec /mnt/SDCARD/App/Sprout/launch.sh' >"$staged" || return 1
    chmod 755 "$staged" || {
        rm -f "$staged"
        return 1
    }
    mv -f "$staged" "$sysdir/cmd_to_run.sh"
}

check_main_ui() {
    if [ ! -f $sysdir/cmd_to_run.sh ]; then
        if sprout_containment_enabled; then
            rm -f /tmp/run_advmenu
            queue_sprout || launch_main_ui
        elif [ -f /tmp/run_advmenu ]; then
            rm /tmp/run_advmenu
...
check_switcher() {
    ...
    elif [ -f "$sysdir/.sprout-handoff" ]; then
        # Sprout atomically replaced its own App command with a validated game.
        rm -f "$sysdir/.sprout-handoff"
    else
        # Return to the idle router (Sprout while contained, MainUI otherwise).
